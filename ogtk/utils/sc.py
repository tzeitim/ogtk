...
def m9tacellize(set_name, 
    ...
    usual_suspects =['shRNA', 'rtTA', 'Fun', 'Cas9', 'Pcna', 'Jun', 'Top2a', 'Txn', 'Hsp90ab1', 'Fos', 'DnaJ', 'Ube2c' ] ,
    ...
    print(f"conquered")
    metacells = mc.pl.collect_metacells(clean, name=f'{set_name}.metacells', random_seed=12345)
    ...
    if 'batch' in clean.obs.columns:
        # TODO switch to pl
        if len(clean.obs['batch'].unique())>1:
    ...
    if return_adatas:
        return([clean, metacells, metadata_df])    
    else:
...
def metacellize(set_name, 
    ...
    with plt.rc_context({"figure.figsize":(15,5)}):
        ...
        plot.set_xlim((0, 1.1*properly_sampled_max_cell_total))    
...
def explore_related_genes(adata, 
    ...
    ret = (pl_var
           ...
           .groupby('related_genes_module').apply(lambda x: heatmap_mod(x))
     )
